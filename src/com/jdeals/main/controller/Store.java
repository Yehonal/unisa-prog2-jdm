...
import com.jdeals.main.helper.discount.RatedDiscount;
...
/**
 * This class is the dataset that store all items and discounts providing all methods to handle the cart,
 * sell the items , requesting price calculation etc.
...
public class Store {
     ...
     * @throws Exception can throw an exception in case a rule with same param. already exists
    ...
    public boolean payOrder() throws Exception {
    	User cUser=this.mainCtrl.getCurUser();
    	/**
    	 * The order price is calculated when user fill the cart
    	 * since would be impolite to change the price as soon as we would pay our cart items
    	 * Instead we should implement a timeout for cart, after which it will be resetted if
    	 * not payed
    	 */
